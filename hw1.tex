...
$$
which is non-negligible.
Hence for both cases we can construct distinguishers against $G$ with non-negligible advantage, implying $G$ is not PRG. Contradiction. Or in other words, for any distinguisher $D$, its advantage
$$\begin{aligned}
&\Adv_{G'',\lambda}^{prg}(D) \\
...
\end{aligned}$$
That is, there exist two distinguishers $D'$ and $D''$, whose advantage against PRG $G$ must be negligible. Hence $\Adv_{G'',\lambda}^{prg}(D)$ must also be negligible.
\end{proof}
