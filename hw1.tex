...
\end{cases}$$
Clearly $f(g(z)) = 0^{2\lambda}$ for all $z\in\bits^{2\lambda}$. Thus $h\eqdef f\circ g$ is \emph{not} one-way over $\bits^{2\lambda}$.
...
If the statistical distance between $P_0$ and $P_1$ is non-negligible, then our distinguisher is efficient. However, if the statistical distance between $P_0$ and $P_1$ is negligible on $\lambda$, then repeating $D$ by feeding independent inputs $x$ by only polynomial times cannot give a non-negligible advantage. Thus if $SD(P_0,P_1)$ is negligible on $\lambda$, our construction of $D$ may not be efficient (i.e., giving non-negligible $\Adv_{P_0,P_1}^{dist}(D)$ in polynomial time).
...
=& \bigg| \Pr[x\getsr\bits^\lambda, b'\getsr D(f(x),P(x)): b'=1] \\
&- 2\Pr[x\getsr\bits^\lambda, u\getsr\bits, b'\getsr D(f(x),u): b'=1]\\
&+\Pr[x\getsr\bits^\lambda, b'\getsr D(f(x),P(x)): b'=1] \bigg|.
\end{aligned}$$
Hence
$$\begin{aligned}
&\Adv_{f,P,\lambda}^{pred}(A) \\
