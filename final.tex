...
$$(X-Y)(X+Y) = 0\mod N.$$
If $X \not= \pm Y \mod N$, then neither $X-Y$ not $X+Y$ is 0 in $\Z_N^*$. Since their product modulo $N$ is 0, which is not in $\Z_N^*$, either $X-Y$ or $X+Y$ is not in $\Z_N^*$. Without loss of generality we assume $X-Y$ is not in $\Z_N^*$. Then $X-Y$ is not coprime with $N$, and thus the gcd of $X-Y$ and $N$ gives the factorization.
\end{proof}
